...
resource "openstack_lb_loadbalancer_v2" "endpoint" {
  name          = "${var.name_prefix}-endpoint"
  vip_subnet_id = openstack_networking_network_v2.private_network.id
}

resource "openstack_lb_listener_v2" "kube_api" {
  name            = "${var.name_prefix}-kube-api"
  loadbalancer_id = openstack_lb_loadbalancer_v2.endpoint.id
  protocol        = "TCP"
...
resource "openstack_networking_floatingip_associate_v2" "endpoint-ip" {
  floating_ip = var.kube_api_external_ip
  port_id     = openstack_lb_loadbalancer_v2.endpoint.vip_port_id
}
