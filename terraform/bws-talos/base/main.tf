...
provider "helm" {
  kubernetes {
    host                   = "https://${var.kube_api_external_ip}"
    cluster_ca_certificate = var.custom_ca_cert
    insecure               = true
  }
}
...
module "talos-config" {
  ...
  cluster_name                     = var.base_name
  cluster_endpoint                 = var.kube_api_external_ip
...
resource "null_resource" "boostrap_trigger" {
  depends_on = [module.instances.controlplane_instance]
}
...
resource "talos_machine_bootstrap" "cluster" {
  ...
  client_configuration = module.talos-config.talos_client_configuration.client_configuration
...
data "talos_cluster_health" "talos" {
  ...
  worker_nodes         = module.network.worker_port_id
